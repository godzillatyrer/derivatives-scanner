...
const CURRENT_VERSION = 2; // Bump to trigger dedup migration
...
export async function loadLearningState() {
  ...
  // One-time migration: deduplicate history from v1
  if (!state.version || state.version < CURRENT_VERSION) {
    const seen = new Map(); // coin+direction → latest entry
    const cleaned = [];
    // Walk newest first so we keep the latest signal per coin+direction
    for (let i = state.history.length - 1; i >= 0; i--) {
      const s = state.history[i];
      if (s.outcome) {
        cleaned.push(s); // resolved signals always kept
        continue;
      }
      ...
      // else: duplicate pending signal → drop it
    }
    state.history = cleaned.reverse();
