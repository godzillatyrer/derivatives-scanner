...
export function generateSignal(timeframeData, weights = DEFAULT_INDICATOR_WEIGHTS) {
  ...
  // Calculate confidence with smarter multi-timeframe conflict handling
  const tfEntries = Object.entries(timeframeResults);
  const directions = tfEntries.map(([, r]) => r.direction);
  ...
  // Check for higher-timeframe conflicts (4h/1d disagreeing with lower TFs)
  const higherTFs = ['4h', '1d'];
  const lowerTFs = ['15m', '1h'];
  const higherDirs = tfEntries.filter(([tf]) => higherTFs.includes(tf)).map(([, r]) => r.direction);
  const lowerDirs = tfEntries.filter(([tf]) => lowerTFs.includes(tf)).map(([, r]) => r.direction);
  const higherBullish = higherDirs.some(d => d.includes('LONG'));
  const higherBearish = higherDirs.some(d => d.includes('SHORT'));
  const lowerBullish = lowerDirs.some(d => d.includes('LONG'));
  const lowerBearish = lowerDirs.some(d => d.includes('SHORT'));
  const htfConflict = (higherBullish && lowerBearish) || (higherBearish && lowerBullish);

  let confidence = Math.abs(compositeScore) * 100;
  if (aligned) {
    confidence *= 1.2;
  } else if (htfConflict) {
    // Higher timeframe disagrees with lower — aggressive penalty
    confidence *= 0.55;
  } else if (directions.some(d => d.includes('LONG')) && directions.some(d => d.includes('SHORT'))) {
    confidence *= 0.7;
  }
...
export function calculateTPSL(signal, candles, learningState = null) {
  ...
  let stopLoss = isLong ? price - slDistance : price + slDistance;

  // TP based on R:R ratios
  let takeProfits = RISK_DEFAULTS.tpLevels.map(rr => {
    const tpDistance = slDistance * rr * (learningState?.rrMultiplier || 1.5);
    return isLong ? price + tpDistance : price - tpDistance;
  });

  // Adjust TP/SL using support & resistance levels
  const sr = indicators.supportResistance;
  if (sr) {
    if (isLong) {
      // Pull SL just below nearest support if it's tighter than ATR-based SL
      if (sr.nearestSupport && sr.nearestSupport > stopLoss && sr.nearestSupport < price) {
        const srSL = sr.nearestSupport * 0.998; // 0.2% below support
        // Only use if it doesn't make the SL too tight (at least 0.3× ATR)
        if (price - srSL >= atr * 0.3) stopLoss = srSL;
      }
      // Cap TP1 if it sits right past resistance (adjust to just below)
      if (sr.nearestResistance) {
        const resistancePct = (sr.nearestResistance - price) / price;
        // If TP1 overshoots nearest resistance by a tiny margin, pull it to resistance
        if (takeProfits[0] > sr.nearestResistance && resistancePct > 0.002) {
          takeProfits[0] = sr.nearestResistance * 0.998;
        }
      }
    } else {
      // Short: pull SL just above nearest resistance
      if (sr.nearestResistance && sr.nearestResistance < stopLoss && sr.nearestResistance > price) {
        const srSL = sr.nearestResistance * 1.002; // 0.2% above resistance
        if (srSL - price >= atr * 0.3) stopLoss = srSL;
      }
      // Cap TP1 to just above nearest support
      if (sr.nearestSupport) {
        const supportPct = (price - sr.nearestSupport) / price;
        if (takeProfits[0] < sr.nearestSupport && supportPct > 0.002) {
          takeProfits[0] = sr.nearestSupport * 1.002;
        }
      }
    }
  }
  ...
  return {
    ...
    atr,
    riskPercent: ((Math.abs(price - stopLoss) / price) * 100).toFixed(2),
  };
}
