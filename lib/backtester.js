 ...
 * Uses train/test split: optimizes on first 70% of data, validates on last 30%.
 */
export function optimizeParameters({
  ...
  const testCandles = candles.slice(splitIdx - WARMUP_BARS); // overlap warmup bars
    ...
    for (const atrMultiplierSL of atrSlRange) {
      for (const rrMultiplier of rrRange) {
        for (const maxHoldBars of maxHoldBarsRange) {
          ...
          if (result.stats.totalTrades < 3) continue;

          trainResults.push({
            params: { minConfidence, atrMultiplierSL, rrMultiplier, maxHoldBars },
            trainStats: result.stats,
          });
        }
      }
    }
  ...
  trainResults.sort((a, b) => {
    const s = a.trainStats, t = b.trainStats;
    const scoreA = s.sharpe * Math.sqrt(s.totalTrades) * Math.min(s.profitFactor, 5);
    const scoreB = t.sharpe * Math.sqrt(t.totalTrades) * Math.min(t.profitFactor, 5);
    return scoreB - scoreA;
  });

  // Validate top 10 candidates on test set
  const results = [];
  for (const candidate of trainResults.slice(0, 10)) {
    ...
    if (testResult.error || testResult.stats.totalTrades < 2) continue;

    // Combined score: average of train and test performance (prevents overfitting)
    const trainScore = candidate.trainStats.sharpe * Math.sqrt(candidate.trainStats.totalTrades) *
      Math.min(candidate.trainStats.profitFactor, 5);
    const testScore = testResult.stats.sharpe * Math.sqrt(testResult.stats.totalTrades) *
      Math.min(testResult.stats.profitFactor, 5);

    results.push({
      params: candidate.params,
      // Use test stats as the "real" performance
      ...testResult.stats,
      trainWinRate: candidate.trainStats.winRate,
      trainPF: candidate.trainStats.profitFactor,
      combinedScore: (trainScore + testScore * 1.5) / 2.5, // weight test more
    });
  }
